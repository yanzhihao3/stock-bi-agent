...
if st.session_state.get('logged', False):
    ...
    # 2. 获取对话列表 API 调用：POST /v1/chat/list（身份由 JWT 决定）
    resp = requests.post(f"{API_BASE_URL}/v1/chat/list", headers=auth_headers()).json()
    if resp.get("code") != 200:
        st.error(resp.get("message", "获取对话列表失败"))
        st.stop()
    chat_data = (resp.get("data") or [])[::-1]  # [::-1] 反转列表，最新的在最上面
